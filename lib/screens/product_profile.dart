...
class _ProductProfilePageState extends State<ProductProfilePage> {
  ...
  void initState() {
    super.initState();
    _fetchSellerVerification();
  }

  Future<void> _fetchSellerVerification() async {
    final sellerId = widget.product['seller_id'] ?? widget.product['sellerID'];
    if (sellerId != null) {
      try {
        ...
        if (mounted && data != null) {
          setState(() {
            _isSellerVerified = data['is_verified'] ?? false;
            _isLoadingVerification = false;
          });
          return;
        }
      } catch (e) {
        debugPrint('Error fetching seller verification: $e');
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final productId = rawId is int ? rawId : rawId is num ? rawId.toInt() : int.tryParse(rawId?.toString() ?? '') ?? 0;
        ...
        child: Column(
          ...
          children: [
            ...
            _ProductReviewsSection(
              productId: productId,
              sellerId: widget.product['seller_id'] as String? ?? '',
            ),
            const SizedBox(height: 24),
            _buildMessageButton(),
  ...
  Widget _buildProductHeader() {
    final productName =
        (widget.product['productName'] as String?)?.trim() ?? 'Unnamed Product';
    ...
    final totalQty = num.tryParse(widget.product['total_quantity']?.toString() ?? '0') ?? 0;
          ...
          children: [
            Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    widget.product['productName'] ?? 'Unnamed',
                    style: const TextStyle(
                        fontSize: 16, fontWeight: FontWeight.bold),
                    overflow: TextOverflow.ellipsis,
                  ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
                'Farmer\'s Location: ${widget.product['location'] ?? 'Unknown'}'),
          ],
        ),
      ),
    );
    ...
    return OutlinedButton(
      onPressed: () {
        if (widget.product['seller_id'] != null &&
            widget.product['sellerName'] != null) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => ChatPage(
                receiverId: widget.product['seller_id'],
                farmerName: widget.product['sellerName'],
              ),
            ),
          );
        }
    ...
    return OutlinedButton(
      onPressed: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => FarmerProfilePage(
              farmerId: widget.product['seller_id'] ?? '',
              farmerName: widget.product['sellerName'] ?? 'Unknown',
            ),
          ),
        );
      },
      ...
      child: const Text('Show Farmer\'s Profile',
          style: TextStyle(color: Colors.green, fontSize: 16)),
    );
  }
}
...
class _ProductReviewsSection extends StatefulWidget {
  ...
  @override
  State<_ProductReviewsSection> createState() =>
...
class _ProductReviewsSectionState extends State<_ProductReviewsSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final isSeller = currentUserId != null && currentUserId == widget.sellerId;
    if (isSeller) {
      return Container(
    ...
    if (_reviews.isEmpty) {
      return const Padding(
        padding: EdgeInsets.all(16),
        child: Text(
          'No reviews yet. Be the first to buy and rate!',
          style: TextStyle(color: Colors.grey),
        ),
      );
    }
    ...
    return Column(
      ...
      children: [
        RatingSummary(
          ...
          onStarFilter: (star) => setState(() => _activeStarFilter = star),
        ),
        ...
        if (_activeStarFilter != null)
          Padding(
            ...
            child: Chip(
              ...
              onDeleted: () => setState(() => _activeStarFilter = null),
        ...
        if (_filteredReviews.isEmpty)
          ...
          ListView.separated(
            ...
            itemBuilder: (_, index) =>
                _buildReviewItem(_filteredReviews[index]),
          ),
      ],
    );
  }
  ...
  Widget _buildReviewItem(Map<String, dynamic> review) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            UserAvatar(avatarUrl: avatarUrl, name: reviewerName, radius: 18),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
